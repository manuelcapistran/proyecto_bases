...
CREATE TABLE PLACA(
    ...
    FECHA_ASIGNACION    DATE DEFAULT  SYSDATE NULL,
    ENTIDAD_ID          NUMBER(10, 0)   NOT NULL,
...
CREATE TABLE PROPIETARIO(
    ...
    RFC                 VARCHAR2(13)    NOT NULL 
        CONSTRAINT PROPIETARIO_RFC_UK UNIQUE,
    CURP                VARCHAR2 (18) 
        CONSTRAINT PROPIETARIO_CURP_UK UNIQUE,
    EMAIL               VARCHAR2(40)    NOT NULL 
        CONSTRAINT PROPIETARIO_EMAIL_UK UNIQUE,
    DERECHO_LICENCIA BOOLEAN DEFAULT TRUE NOT NULL,
...
CREATE TABLE LICENCIA (
    ...
    FOTO                BLOB            NOT NULL,
    FIRMA               BLOB            NOT NULL,
    HUELLA_INDICE_DER   BLOB            NOT NULL, -- Ambas huellas incluidas
    HUELLA_INDICE_IZQ   BLOB            NOT NULL,
    NUM_LICENCIA        VARCHAR2(10)    NOT NULL,
    CONSTRAINT PROPIETARIO_NUM_LICENCIA_UK UNIQUE (NUM_LICENCIA),
    TIPO                VARCHAR2(1)     NOT NULL,
    CONSTRAINT LICENCIA_TIPO_CHK CHECK (TIPO IN ('A', 'B', 'C')),
    FECHA_EMISION       DATE DEFAULT SYSDATE NOT NULL,
    FECHA_VENCIMIENTO   DATE GENERATED ALWAYS AS (
        CASE 
            WHEN TIPO IN ('A', 'B') THEN ADD_MONTHS(FECHA_EMISION, 36) -- 3 años
            WHEN TIPO = 'C' THEN ADD_MONTHS(FECHA_EMISION, 48)         -- 4 años
        END
    ) STORED NOT NULL,
    DIAS_PARA_VENCIMIENTO NUMBER GENERATED ALWAYS AS (FECHA_VENCIMIENTO - FECHA_EMISION) VIRTUAL,
    PROPIETARIO_ID NUMBER(10, 0) NOT NULL,
...
CREATE TABLE VEHICULO(
    ...
    CONSTRAINT VEHICULO_TIPO_CHK CHECK ( NOT ES_DE_CARGA        AND NOT 
                                         ES_PARTICULAR          AND
                                         ES_TRANSPORTE_PUBLICO  OR 
                                         ES_DE_CARGA            AND NOT 
                                         ES_TRANSPORTE_PUBLICO  OR 
                                         ES_PARTICULAR          AND NOT 
                                         ES_TRANSPORTE_PUBLICO )
);

--DROP table if EXISTS de_carga;
CREATE TABLE DE_CARGA (
    VEHICULO_ID NUMBER(10, 0) NOT NULL,
    NUM_REMOLQUES NUMBER(1, 0) DEFAULT NULL,
...
CREATE TABLE VERIFICACION (
    ...
    FECHA_VERIFICACION DATE DEFAULT SYSDATE NOT NULL, -- Corregido el orden
    FOLIO VARCHAR2(13) NOT NULL
    CONSTRAINT VERIFICACION_FOLIO_UK UNIQUE,
    CLAVE_VERIFICENTRO VARCHAR2(5) NOT NULL,
    VEHICULO_ID NUMBER(10, 0) NOT NULL,
    CONSTRAINT VERIFICACION_PK PRIMARY KEY (VERIFICACION_ID),
    CONSTRAINT VERIFICACION_VEHICULO_ID_FK FOREIGN KEY (VEHICULO_ID) REFERENCES VEHICULO(VEHICULO_ID)
);
...
CREATE TABLE GAS (
    ...
    NIVELES_PERMITIDOS NUMBER(4,4) NOT NULL,
...
CREATE TABLE REPORTE_VERIFICENTRO(
    ...
    CONSTRAINT REPORTE_VERIFICENTRO_GAS_ID_FK FOREIGN KEY (GAS_ID) 
        REFERENCES GAS(GAS_ID),
    CONSTRAINT REPORTE_VERIFICENTRO_VERIFICACION_ID_FK FOREIGN KEY (VERIFICACION_ID) 
...
CREATE TABLE REPORTE_EMISIONES(
    ...
    CONSTRAINT REPORTE_EMISIONES_GAS_ID_FK FOREIGN KEY (GAS_ID) REFERENCES GAS(GAS_ID),
    CONSTRAINT REPORTE_EMISIONES_VEHICULO_ID_FK FOREIGN KEY (VEHICULO_ID) REFERENCES VEHICULO(VEHICULO_ID)
);
...
CREATE TABLE REPORTE_EMISIONES_NOTIFICACION(
    ...
    NOTIFICACION_ID NUMBER(10, 0) NOT NULL,
    CONSTRAINT REPORTE_EMISIONES_NOTIFICACION_REPORTE_EMISIONES_ID_FK FOREIGN KEY (REPORTE_EMISIONES_ID)
        REFERENCES REPORTE_EMISIONES(REPORTE_EMISIONES_ID),
    CONSTRAINT REPORTE_EMISIONES_NOTIFICACION_REPORTE_NOTIFICACION_ID_FK FOREIGN KEY (NOTIFICACION_ID) REFERENCES NOTIFICACION(NOTIFICACION_ID),
...
CREATE TABLE HISTORICO_PROPIETARIO_VEHICULO(
    HISTORICO_PROPIETARIO_VEHICULO_ID NUMERIC(10, 0) CONSTRAINT HISTORICO_PROPIETARIO_VEHICULO_PK PRIMARY KEY,
    FECHA_COMPRA DATE NOT NULL,
    FECHA_VENTA DATE NULL,
    VEHICULO_ID NUMERIC(10, 0) not null,
    PROPIETARIO_ID NUMERIC(10, 0) not null,
